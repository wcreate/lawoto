...
(function() {
    ...
    var canSendBinaryString = (
        (window.XMLHttpRequest && window.XMLHttpRequest.prototype.sendAsBinary) || (window.Blob && window.Uint8Array && window.ProgressEvent) || (window.ArrayBuffer && window.BlobBuilder)
    );
    ...
    var isSupported = (
        (canReadAsBinaryString && canSendBinaryString) || (canReadFormData && canSendFormData)
    );
    var isImageSupported = (
        canReadAsBase64 && (
            (canResizeImageToBinaryString && canSendBinaryString) || (canResizeImageToFile && canSendFormData)
        )
    );
